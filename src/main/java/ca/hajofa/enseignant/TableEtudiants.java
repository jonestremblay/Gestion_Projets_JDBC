...
public abstract class TableEtudiants {
     ...
     */
    public static void ajouterData(List<Etudiant> liste, JTable tableEtudiants) {
     ...
     * @param comboBoxCours
     */
    public static void setValeurs(List<Etudiant> liste, JComboBox comboBoxCours) {

        for (Etudiant etud : liste) {
            String des = etud.getId_Etudiant()
     ...
     * @param tableCours
     */
    public static void setValeursTableEtudiants(List<Etudiant> liste, JTable tableCours) {
        DefaultTableModel modele = (DefaultTableModel) tableCours.getModel();
        Object data[] = new Object[2];
        for (int i = 0; i < liste.size(); i++) {
            data[0] = liste.get(i).getId_Etudiant();
            data[1] = liste.get(i).getPrenom() + " "
    ...
    public static boolean validerAjoutEtudiant(JTextField nom, JTextField prenom, 
        ...
        }else if(!(TableEtudiants.validerCourriel(courriel.getText()))){
            flag = false;
