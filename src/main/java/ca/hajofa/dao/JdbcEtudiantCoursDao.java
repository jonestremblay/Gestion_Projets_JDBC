 ...
 */
public class JdbcEtudiantCoursDao implements EtudiantCoursDao{
    ...
    public boolean deleteCoursEtudiant(String email) {
        JdbcEtudiantDao dao = new JdbcEtudiantDao();
        ...
        */
        try {
            ...
            while(res.next()) {
                ...
                JdbcEtudiantCoursDao dao = new JdbcEtudiantCoursDao();
                dejaInscrit = dao.verifierInscriptionCours(IDCoursTrouve, 
                                             EtudiantConnecte.idEtudiant);
                if (!dejaInscrit){
                    // l'ajouter au cours. 
                    String message = "<html><body>Vous avez été ajouté au cours " + 
                                     titreCoursTrouve + "</body></html>";
                    FenEtudiant.getMessageLabelOfDialogPane("succes")
                            .setText(message);
                    resultat = 1;
                } else {
                    // tu es deja inscrit ! 
                     String message = "<html><body>Vous êtes déjà dans le cours " + 
                                     titreCoursTrouve + "</body></html>";
                    FenEtudiant.getMessageLabelOfDialogPane("oups")
                            .setText(message);
                    resultat = 0;
                }
                }
            ...
            Logger.getLogger(JdbcEtudiantCoursDao.class.getName()).log(Level.SEVERE, null, ex);
        } 
        Database.close();
        return resultat;
    }
