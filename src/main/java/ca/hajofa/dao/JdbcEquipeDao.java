 ...
 */
public class JdbcEquipeDao implements EquipeDao{
    ...
    @Override
    public ArrayList<Equipe> findAllByIdCours(int idCours) {
        Connection cnx = null;
        PreparedStatement prepStm = null;
        ResultSet res = null;
        ArrayList<Equipe> listeEquipes = new ArrayList<Equipe>();
        ...
        try {
            ...
            prepStm.setInt(2, EtudiantConnecte.idEtudiant);
            res = prepStm.executeQuery();
   ...
   /**
    ...
    public boolean create(String cours, String nomEquipe) {
        ...
        if (findByIdNom(idCours, nomEquipe).getNomEquipe() != null) {
            cree = false;
            JOptionPane.showMessageDialog(null, nomEquipe + " existe déjà!",
                    "Equipe existe",
    ...
    /**
     *La methode findByIdCoursl() permet cherhcher le id d'un cours
     * @param cours
     * @return
     */
    public List<Equipe> findByIdCours(String cours) {
        List<Equipe> liste = new ArrayList<>();
        String requete = "SELECT * FROM Equipes e JOIN Cours c ON(e.ID_Cours=c.ID_Cours) WHERE aPropos=?";
        PreparedStatement stm;
        Connection cnx = null;
        try {
            cnx = Database.getConnexion();
